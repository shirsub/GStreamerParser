    ...
    GstURIDecodeBin * decoder)
{
  ...
  GST_ELEMENT_WARNING (decoder, STREAM, CODEC_NOT_FOUND,
      (_("No nach gsturidecodebin 931 decoder available for type \'%s\'."), capsstr), (NULL));
  g_free (capsstr);
}
    ...
    if (!decoder->streams || g_hash_table_size (decoder->streams) == 0) {
      if (decoder->missing_plugin_errors) {
        ...
        GST_ELEMENT_ERROR (decoder, CORE, MISSING_PLUGIN, (NULL),
            ("no suitable nach gstdecbin 998 plugins found:\n%s", str->str));
        g_string_free (str, TRUE);
      } else {
        GST_ELEMENT_ERROR (decoder, CORE, MISSING_PLUGIN, (NULL),
            ("no suitable nach gstdecbin 1002 plugins found"));
      }
    } else {
